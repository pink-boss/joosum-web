...
describe("캘린더 날짜 렌더링", () => {
  test("2024.12.", () => {
    const mocks_2024_12 = new Array(31)
      .fill(1)
    ...
    expect(getCalendarDate(date).map(({ date }) => date)).toEqual(
      monthDateMocks_2024_12,
    );
  });

  test("2025.01.", () => {
    const mocks_2024_12 = [29, 30, 31];
    const mocks_2025_01 = new Array(31)
      .fill(1)
      .map((date, index) => date + index);
    const mocks_2025_02 = [1];
    const monthDateMocks_2025_01 = [
      ...mocks_2024_12,
      ...mocks_2025_01,
      ...mocks_2025_02,
    ];
...
describe("날짜 비교", () => {
  test("동등", () => {
    expect(
      isSameDate(new Date("2024.12.10"), new Date("2024.12.10")),
    ).toBeTruthy();
    expect(
      isSameDate(new Date("2024.12.10"), new Date("2025.12.10")),
    ).toBeFalsy();
  });
